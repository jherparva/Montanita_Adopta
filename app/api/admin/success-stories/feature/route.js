...
// PUT - Marcar/desmarcar una historia como destacada (solo para administradores)
export async function PUT(request) {
  try {
    ...
    // Actualizar el estado de destacado de la historia
    const updatedStory = await Story.findByIdAndUpdate(storyId, { isFeatured }, { new: true });
    ...
    return NextResponse.json({
      success: true,
      message: isFeatured ? "Historia marcada como destacada" : "Historia desmarcada como destacada",
      story: updatedStory,
    });
  } catch (error) {
    console.error("Error al actualizar historia destacada:", error);
