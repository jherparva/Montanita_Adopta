...
// PUT - Marcar/desmarcar una historia como testimonio destacado (solo para administradores)
export async function PUT(request) {
  try {
    ...
    const { storyId, isTestimony } = body;
    ...
    // Actualizar el estado de testimonio de la historia
    const updatedStory = await Story.findByIdAndUpdate(storyId, { isTestimony }, { new: true });
    ...
    return NextResponse.json({
      success: true,
      message: isTestimony ? "Historia marcada como testimonio destacado" : "Historia desmarcada como testimonio destacado",
      story: updatedStory,
    });
  } catch (error) {
    console.error("Error al actualizar testimonio destacado:", error);
