...
// Función para manejar las solicitudes PUT (marcar como leído)
export async function PUT(req, { params }) {
  try {
    ...
    const updateResult = await db.collection("messages").updateOne(
      ...
      { $set: { read: true, readBy: userId, readAt: new Date() } }
    )

    if (updateResult.matchedCount === 0) {
      return NextResponse.json({ message: "No se encontró el mensaje" }, { status: 404 })
    }
